...
async function videoToMp3(){
 ...
 <button class="primary" id="convert" disabled>MP3으로 변환</button>
 <div class="progress"><i id="bar"></i></div>
 <p class="convert-status" id="status">파일을 선택하면 변환할 수 있어요.</p>
 <button class="secondary" id="download" disabled style="width:100%;margin-top:8px">저장</button>`);
 $('#close').onclick=closeModal;
 ...
 $('#convert').onclick=async()=>{
   ...
   try{
     ...
     await new Promise((resolve,reject)=>{
       v.ontimeupdate=()=>{
         const pct=Math.min(95,(v.currentTime/total)*95);
         if(pct-last>=1){$('#bar').style.width=pct+'%';last=pct}
         if(v.ended || v.currentTime>=total-0.05){v.pause();rec.stop();resolve()}
       };
       v.onerror=reject;
       v.onended=()=>{if(rec.state!=='inactive')rec.stop();resolve()};
     });
     await new Promise(r=>{if(rec.state==='inactive')r();else rec.onstop=r});
     const audioBlob=new Blob(chunks,{type:'audio/webm'});
     const buf=await audioBlob.arrayBuffer();
     const decoded=await ctx.decodeAudioData(buf);
     const samples=decoded.getChannelData(0);
     const enc=new lamejs.Mp3Encoder(1,decoded.sampleRate,128), block=1152, mp3=[];
     for(let i=0;i<samples.length;i+=block){
       const end=Math.min(i+block,samples.length), pcm=new Int16Array(end-i);
       for(let j=i;j<end;j++) pcm[j-i]=Math.max(-1,Math.min(1,samples[j]))*32767;
       const b=enc.encodeBuffer(pcm); if(b.length)mp3.push(new Int8Array(b));
       $('#bar').style.width=(95+(i/samples.length)*5)+'%';
       await new Promise(requestAnimationFrame);
     }
     const tail=enc.flush(); if(tail.length)mp3.push(new Int8Array(tail));
     const out=new Blob(mp3,{type:'audio/mpeg'});
     const url=URL.createObjectURL(out);
     $('#bar').style.width='100%';
     $('#status').textContent='변환 완료! 저장 버튼을 눌러주세요.';
     $('#download').disabled=false;
     $('#download').onclick=()=>{
       const a=document.createElement('a'); a.href=url; a.download=f.name.replace(/\.[^.]+$/,'')+'.mp3'; a.click();
       toast('MP3 저장을 시작했어요.');
     };
     await ctx.close();
   }catch(e){
     console.error(e);
     $('#status').textContent='이 브라우저에서 변환에 실패했어요. 다른 동영상으로 다시 시도해주세요.';
     $('#convert').disabled=false;
   }
 };
}
...
(async()=>{await openDB();albums=await getAll();renderHome();setTimeout(()=>$('#splash')?.classList.add('hide'),1700)})();
